      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Clock className="text-primary-600" size={26} /> Overtime Management
          </h1>
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-800">
          ...
          <ul className="list-disc pl-4 mt-1 space-y-0.5 text-blue-700/80">
            <li>
              Employee MUST have their attendance marked for the date first.
            </li>
      ...
      {recentOvertimes.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-2">
            ...
            <h3 className="font-black text-gray-700 uppercase tracking-widest text-xs">
              Monthly Records
            </h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap min-w-full">
              <thead className="bg-gray-50/30 text-gray-400 border-b border-gray-100">
                <tr>
                  <th className="px-4 py-3 font-bold text-[10px] uppercase tracking-wider">
                    Date
                  </th>
                  <th className="px-4 py-3 font-bold text-[10px] uppercase tracking-wider">
                    Units
                  </th>
                  <th className="px-4 py-3 font-bold text-[10px] uppercase tracking-wider text-center">
                    Status
                  </th>
                  <th className="px-4 py-3 font-bold text-[10px] uppercase tracking-wider text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {recentOvertimes.map((ot: any) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-black text-primary-600">
                      +{ot.units}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-tighter border ${getStatusStyle(ot.status || "PENDING")}`}
                      >
                        {ot.status || "PENDING"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      {(ot.status === "PENDING" || !ot.status) && (
                        <div className="flex justify-end gap-1">
                          <button
                            onClick={() =>
                              handleApproveReject(ot._id, "APPROVED")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleApproveReject(ot._id, "REJECTED")
                            }
            ...
            <tbody className="divide-y divide-gray-100">
              {allOvertimes.length === 0 ? (
                <tr>
                  ...
                  >
                    No overtimes recorded for this date.
                  </td>
                </tr>
              ) : (
                allOvertimes.map((ot) => (
                  <tr key={ot._id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase border tracking-tighter ${getStatusStyle(ot.status || "PENDING")}`}
                      >
                        {ot.status || "PENDING"}
                      </span>
                    </td>
