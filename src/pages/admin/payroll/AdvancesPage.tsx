...
export const AdvancesPage = () => {
    ...
    const [viewMonth, setViewMonth] = useState<string>(new Date().toISOString().substring(0, 7));
    ...
    const exportCSV = () => {
        ...
        const headers = 'Employee Name,Employee ID,Disbursement Date,Amount,Status,Remarks\n';
        ...
        link.href = url;
        link.download = `Advances_${viewMonth}.csv`;
        link.click();
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="p-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-semibold text-gray-700">History for {viewMonth}</h3>
                        </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {recentAdvances.map((adv: any) => (
                                    <tr key={adv._id}>
                                        ...
                                        <td className="px-4 py-3 font-medium text-gray-900">₹{adv.amount.toLocaleString()}</td>
                                        <td className="px-4 py-3 text-emerald-600 font-bold">₹{(adv.repaidAmount || 0).toLocaleString()}</td>
                                        <td className="px-4 py-3 text-red-600 font-bold">₹{(adv.amount - (adv.repaidAmount || 0)).toLocaleString()}</td>
                                        <td className="px-4 py-3 text-center">
                                            <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                                adv.status === 'PAID' ? 'bg-emerald-100 text-emerald-700' :
                                                adv.status === 'PARTIAL' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                                                {adv.status || 'Pending'}
                                            </span>
                                        </td>
                ...
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm font-semibold text-primary-700 focus:ring-2 focus:ring-primary-500 outline-none"
                        />
                        <button
                            onClick={exportCSV}
                    ...
                    <table className="w-full text-left text-sm whitespace-nowrap min-w-full">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {allAdvances.length === 0 ? (
                                <tr>
                                    <td colSpan={7} className="px-4 py-8 text-center text-gray-500">No advances recorded for this month.</td>
                                </tr>
                            ) : (
