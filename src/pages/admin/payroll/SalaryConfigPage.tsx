...
import { Plus, Download, Table2, Settings2, Eye, X, ArrowRight } from 'lucide-react';
import {
    createSalaryConfig,
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-gray-700">Salary Config Records</h3>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            type="button"
                                ...
                                allConfigs.map((config) => (
                                    <tr key={config._id} className="hover:bg-gray-50/50">
                                        <td className="px-4 py-3">
                                            <div className="font-medium text-gray-900">{config.employeeId?.name || '-'}</div>
                                            <div className="text-[10px] text-gray-400 uppercase font-black tracking-widest">
                                                {config.employeeId?.department || 'Employee'}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-gray-500">{config.employeeId?.empId || '-'}</td>
                                        <td className="px-4 py-3 text-gray-500">
                                            <div className="font-semibold text-gray-900">{formatMonthLabel(config.effectiveFrom)}</div>
                                            <div className="text-[10px] text-gray-400">Starts {formatDate(config.effectiveFrom)}</div>
                                        </td>
                                        <td className="px-4 py-3 font-bold text-primary-700">Rs. {Number(config.monthlySalary || 0).toLocaleString()}</td>
                                        <td className="px-4 py-3">
                                            <span
                                                className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                                    config.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                                                }`}
                                            >
            ...
            {isGenerateModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={closeGenerateModal} />
                    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-3xl lg:max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-white">
                        <div className="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 text-white flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h3 className="text-xl sm:text-2xl font-bold text-white mt-1">Salary Configuration</h3>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                                <div className="rounded-3xl border border-gray-100 bg-gray-50/80 p-5 sm:p-6 space-y-5">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">Select Employee</label>
                                            <select
                                                value={selectedUser}
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">Effective From</label>
                                            <input
                                                type="date"
                                                value={effectiveFrom}
                                                ...
                                                required
                                            />
                                            <p className="text-xs text-gray-500">Salary becomes active from this date.</p>
                                        </div>
                                    </div>

                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-[0.25em] text-gray-400">New Monthly Salary (Rs.)</label>
                                        <div className="relative">
                                            <span className="absolute left-4 top-3 text-gray-500 font-black">Rs.</span>
                                            <input
                                                type="number"
                                                min="1"
                                                value={monthlySalary}
                                ...
                                <div className="rounded-3xl border border-violet-100 bg-violet-50/60 p-5 sm:p-6 space-y-4">
                                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-violet-500">Preview</p>
                                    ...
                                    <div className="rounded-2xl bg-white border border-violet-100 p-4 shadow-sm">
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Effective Date</p>
                                        <p className="mt-2 text-base font-bold text-gray-900">{formatDate(effectiveFrom)}</p>
                                        <p className="text-xs font-bold text-gray-500 mt-1">Shows as {formatMonthLabel(effectiveFrom)}</p>
                                    </div>
                                    <div className="rounded-2xl bg-white border border-violet-100 p-4 shadow-sm">
                                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Salary</p>
                                        <p className="mt-2 text-base font-bold text-gray-900">
                                            Rs. {monthlySalary ? Number(monthlySalary).toLocaleString() : '0'}
                                        </p>
                                        <p className="text-xs font-bold text-gray-500 mt-1">Monthly base salary</p>
                                    </div>
                                </div>
                            </div>
            ...
            {isViewDrawerOpen && selectedConfig && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={closeConfigView} />
                    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-3xl lg:max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 text-white flex items-start justify-between gap-4">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.35em] text-violet-100/80">Salary Config Details</p>
                                <h3 className="text-xl sm:text-2xl font-bold text-white mt-1">{selectedConfig.employeeId?.name || 'Employee'}</h3>
                                <p className="text-sm text-violet-100/80 mt-1 max-w-2xl">
                                    {selectedConfig.employeeId?.empId || 'N/A'} {selectedConfig.effectiveFrom ? `- Effective ${formatMonthLabel(selectedConfig.effectiveFrom)}` : ''}
                                </p>
                            </div>
                            <button
                        ...
                        <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
                            {viewLoading ? (
                                <div className="py-16 text-center text-gray-400 font-medium animate-pulse">Loading salary config details...</div>
                            ) : (
                                <>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                                            <p className="text-[10px] uppercase tracking-[0.25em] text-gray-400 font-black">Selected Salary</p>
                                            <p className="text-2xl font-bold text-violet-700 mt-2">Rs. {Number(selectedConfig.monthlySalary || 0).toLocaleString()}</p>
                                        </div>
                                        <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                                            <p className="text-[10px] uppercase tracking-[0.25em] text-gray-400 font-black">Status</p>
                                            <p className="text-lg font-bold mt-2 text-gray-900">{selectedConfig.isActive ? 'ACTIVE' : 'INACTIVE'}</p>
                                            <p className="text-xs text-gray-500 mt-1">
                                                Updated {formatDate(selectedConfig.updatedAt || selectedConfig.createdAt || selectedConfig.effectiveFrom)}
                                            </p>
                                        </div>
                                        <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                                            <p className="text-[10px] uppercase tracking-[0.25em] text-gray-400 font-black">Created By</p>
                                            <p className="text-lg font-bold mt-2 text-gray-900">{selectedConfig.createdBy?.name || 'System'}</p>
                                            <p className="text-xs text-gray-500 mt-1">Effective month {formatMonthLabel(selectedConfig.effectiveFrom)}</p>
                                        </div>
                                    </div>

                                    <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
                                        <h4 className="text-sm font-black text-blue-900 uppercase tracking-[0.2em]">Payment Snapshot</h4>
                                        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                                            <div className="rounded-xl bg-white border border-blue-100 p-3">
                                                <p className="text-[10px] uppercase text-blue-500 font-black tracking-widest">Base Salary</p>
                                                <p className="text-base font-bold text-blue-900 mt-1">Rs. {Number(viewCurrentSalary?.monthlySalary ?? selectedConfig.monthlySalary ?? 0).toLocaleString()}</p>
                                            </div>
                                            <div className="rounded-xl bg-white border border-blue-100 p-3">
                                                <p className="text-[10px] uppercase text-blue-500 font-black tracking-widest">Effective Month</p>
                                                <p className="text-base font-bold text-blue-900 mt-1">
                                                    {formatMonthLabel(viewCurrentSalary?.effectiveFrom || selectedConfig.effectiveFrom)}
                                                </p>
                                            </div>
                                            <div className="rounded-xl bg-white border border-blue-100 p-3">
                                                <p className="text-[10px] uppercase text-blue-500 font-black tracking-widest">Employee</p>
                                                <p className="text-base font-bold text-blue-900 mt-1">{selectedConfig.employeeId?.name || '-'}</p>
                                            </div>
                                            <div className="rounded-xl bg-white border border-blue-100 p-3">
                                                <p className="text-[10px] uppercase text-blue-500 font-black tracking-widest">Emp ID</p>
                                                <p className="text-base font-bold text-blue-900 mt-1">{selectedConfig.employeeId?.empId || '-'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="rounded-2xl border border-gray-100 overflow-hidden">
                                        <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                                            <h4 className="text-sm font-black text-gray-800 uppercase tracking-[0.2em]">Salary History</h4>
                                            <button
                                                type="button"
                                        ...
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-sm">
                                                <thead className="bg-gray-50/50 text-gray-400">
                                                    <tr>
                                                        <th className="px-5 py-3 text-left font-black text-[10px] uppercase tracking-wider">Monthly Salary</th>
                                                        <th className="px-5 py-3 text-left font-black text-[10px] uppercase tracking-wider">Effective Month</th>
                                                        <th className="px-5 py-3 text-center font-black text-[10px] uppercase tracking-wider">Status</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="divide-y divide-gray-100">
                                                    {viewHistory.length === 0 ? (
                                                        ...
                                                        viewHistory.map((record: any) => (
                                                            <tr key={record._id} className="hover:bg-gray-50/50">
                                                                <td className="px-5 py-3 font-bold text-gray-900">Rs. {Number(record.monthlySalary || 0).toLocaleString()}</td>
                                                                <td className="px-5 py-3 text-gray-600">{formatMonthLabel(record.effectiveFrom)}</td>
