        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:hidden">
                {isLoadingRecords ? (
                    ...
                    <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200 text-gray-500 text-sm font-medium">No records found for this month</div>
                ) : (
                    records.map((record) => (
                        <div key={record._id} className={`bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4 ${record.status === 'PAID' ? 'border-emerald-100 bg-emerald-50/5' : ''}`}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-12 h-12 rounded-2xl bg-primary-100 flex items-center justify-center text-primary-700 font-black text-lg uppercase shadow-sm">
                                        {record.employeeId?.name ? record.employeeId.name.charAt(0) : '?'}
                                    </div>
                                    <div>
                                        <p className="font-black text-gray-900 leading-tight">{record.employeeId?.name || 'Unknown'}</p>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">{record.employeeId?.empId || 'N/A'}</p>
                                    </div>
                                </div>
                                <select
                                    value={record.status}
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-tighter border outline-none cursor-pointer ${
                                        record.status === 'PAID'
                            ...
                            <div className="grid grid-cols-2 gap-3 py-3 border-y border-gray-50">
                                <div>
                                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.1em] mb-1">Attendance</p>
                                    {(() => {
                                        const summary = getAttendanceSummary(record.employeeId?._id || record.employeeId);
                                        return (
                                            <div className="flex flex-wrap gap-1">
                                                <span className="px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600 text-[8px] font-black border border-emerald-100">P{summary.presentDays}</span>
                                                <span className="px-1.5 py-0.5 rounded bg-red-50 text-red-600 text-[8px] font-black border border-red-100">A{summary.absentDays}</span>
                                                <span className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 text-[8px] font-black border border-amber-100">L{summary.leaveDays}</span>
                                            </div>
                                        );
                                    })()}
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.1em] mb-1">Net Payout</p>
                                    <p className="text-lg font-black text-primary-700 leading-none">₹{Math.round(record.netSalary || 0).toLocaleString()}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex-1 py-2.5 bg-gray-50 text-gray-700 rounded-xl font-bold text-[10px] uppercase border border-gray-100 active:scale-95 transition-all"
                                >
                                    View Details
                                </button>
                                <button
                                    onClick={() => downloadSalarySlip(record)}
                                    className="flex-1 py-2.5 bg-primary-600 text-white rounded-xl font-bold text-[10px] uppercase shadow-md shadow-primary-100 active:scale-95 transition-all"
                                >
                                    Print Slip
                                </button>
                                <button
