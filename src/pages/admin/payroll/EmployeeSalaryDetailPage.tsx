...
export const EmployeeSalaryDetailPage = () => {
  ...
  const totalRepaid = salaryRecords.reduce(
    (s, r) => s + (r.advanceTotal || 0),
    0,
  );
  const totalOutstanding = totalAdvanceTaken - totalRepaid;

  const empName =
    salaryRecords[0]?.employeeId?.name || employee?.name || "Employee";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5 font-semibold uppercase tracking-widest">
            {empId} &bull; Salary & Advance History
          </p>
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-black text-red-600 mt-0.5">
                  ₹{totalAdvanceTaken.toLocaleString()}
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-sm text-left whitespace-nowrap">
                    ...
                    salaryRecords.map((rec) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-semibold text-gray-900">
                          ₹{(rec.baseSalary || 0).toLocaleString()}
                        </td>
                        ...
                        <td className="px-4 py-3 font-black text-primary-700">
                          ₹{Math.round(rec.netSalary || 0).toLocaleString()}
                        </td>
                      </tr>
                    ))
                  )}
                ...
                {salaryRecords.length > 0 && (
                  <tfoot className="bg-gray-50 border-t border-gray-200">
                    <tr>
                      <td className="px-4 py-3 text-[10px] font-black uppercase text-gray-500 tracking-wider">
                        Totals
                      </td>
                      <td className="px-4 py-3 font-black text-gray-900">
                        ₹
                        {salaryRecords
                          .reduce((s, r) => s + (r.baseSalary || 0), 0)
                          .toLocaleString()}
                      </td>
                      ...
                      <td className="px-4 py-3 font-black text-emerald-600">
                        ₹
                        {salaryRecords
                          .reduce((s, r) => s + (r.advanceTotal || 0), 0)
                          .toLocaleString()}
                      </td>
                      <td className="px-4 py-3 font-black text-primary-700">
                        ₹
