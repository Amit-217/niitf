...
export const MyTasksPage = () => {
  ...
  const currentUserId =
    currentUser?.userId || currentUser?.id || currentUser?._id || null;
  ...
  const openTaskModal = async (
    task: Task,
    initialStatus?: "ASSIGNED" | "IN_PROGRESS" | "COMPLETED",
  ) => {
  ...
  const handleArchiveTask = async (taskId: string) => {
    if (
      !window.confirm(
        "Are you sure you want to archive this task? It will disappear from active/completed lists.",
      )
    )
      return;
    try {
    ...
    updates.length === 0 ? (
      <div className="flex flex-col items-center justify-center py-12 px-4 bg-white rounded-3xl border border-dashed border-gray-200">
        ...
        <p className="text-sm text-gray-400 font-medium text-center">
          No updates posted in this thread yet.
        </p>
      </div>
    ) : (
      <div className="flex flex-col-reverse space-y-4 space-y-reverse flex-1 lg:overflow-y-auto pr-1">
        {updates.map((update: any) => {
          ...
          return (
            ...
            >
              <div
                className={`max-w-[92%] sm:max-w-[85%] rounded-2xl border px-4 py-3 shadow-sm ${
                  isMine
              ...
              >
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <span
                    className={`text-[9px] font-bold uppercase tracking-wider ${isMine ? "text-violet-100" : "text-gray-400"}`}
                  >
                    {new Date(
                ...
                <p
                  className={`text-xs sm:text-sm whitespace-pre-wrap leading-relaxed ${isMine ? "text-violet-50" : "text-gray-700"}`}
                >
                  {update.comment}
                </p>
              </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Only tasks assigned to your admin account appear here.
            </p>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="py-20 flex flex-col items-center justify-center bg-white rounded-3xl border border-dashed border-gray-200 text-gray-400">
          ...
          <p className="text-xs mt-1">
            Try changing your filters or contact your admin.
          </p>
        </div>
      ) : (
          ...
          {filteredTasks.map((task) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <span
                  className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${getStatusBadge(task.status)}`}
                >
            ...
            <div className="px-6 py-5 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 text-white flex justify-between items-start gap-4">
              <div>
                ...
                <p className="text-violet-50 text-xs mt-1">
                  Review instructions and submit updates or finalize your
                  personal task.
                </p>
              </div>
              <button
              ...
              <div className="p-6 lg:overflow-y-auto border-b lg:border-b-0 lg:border-r border-gray-100">
                <div className="rounded-2xl border border-gray-100 bg-gray-50/50 p-4 mb-6">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <span
                      className={`px-2.5 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${getStatusBadge(selectedTask.status)}`}
                    >
