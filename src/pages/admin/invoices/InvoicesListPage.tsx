...
import {
    Plus, Search, FileText, Pencil, Eye, Trash2, Loader2, Receipt
} from 'lucide-react';
...
export const InvoicesListPage: React.FC = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await getAllInvoices({ page, limit, search: search || undefined, status: statusFilter || undefined });
            const body: any = res;
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    ...
                                    <th className="px-4 py-3 text-right font-semibold text-gray-600">Grand Total</th>
                                ...
                                {invoices.map((inv: any) => (
                                    <tr key={inv._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[inv.status] || 'bg-gray-100 text-gray-600'}`}>
                                                {inv.status || 'Draft'}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-center gap-2">
                                                <button
                                                    onClick={() => navigate(`${basePath}/invoices/${inv._id}/print`)}
                                                    title="View / Print"
                                                ...
                                                <button
                                                    onClick={() => navigate(`${basePath}/invoices/${inv._id}/edit`)}
                                                    title="Edit"
