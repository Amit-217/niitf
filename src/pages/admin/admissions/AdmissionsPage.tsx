...
export const AdmissionsPage = () => {
  ...
  const [feeForm, setFeeForm] = useState<FeePaymentPayload>({
    installmentNo: 1,
    amount: 0,
    paymentMode: "Cash",
  });
  ...
  useEffect(() => {
    ...
    if (pending) {
      ...
      toast.info(
        `Converting enquiry for ${data.studentName}. Please select the student record.`,
      );
  ...
  useEffect(() => {
    const total =
      parseFloat(form.totalFees || "0") - parseFloat(form.discount || "0");
  ...
  const handleComplete = async (id: string) => {
    if (
      !window.confirm(
        "Mark this admission as Completed? This requires full payment.",
      )
    )
      return;
    try {
      ...
      toast.error(err?.message || err?.error || "Cannot complete");
    }
  };
                ...
                admissions.map((a) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {a.courseId?.courseName || a.courseId?.title}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-bold border ${STATUS_BADGE[a.status] || ""}`}
                      >
                        {a.status}
                      </span>
                    </td>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-100 text-sm mt-0.5">
                    Link a student to a course & batch to start.
                  </p>
                </div>
                <button
                  onClick={() => setCreateOpen(false)}
            ...
            <form onSubmit={handleCreate} className="flex flex-col h-[80vh]">
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2 p-5 bg-gray-900 rounded-2xl flex items-center justify-between border-b-4 border-indigo-600 shadow-xl overflow-hidden relative group">
                      ...
                      <BadgeDollarSign
                        size={48}
                        className="text-white/5 absolute -right-2 top-0 group-hover:scale-110 transition-transform duration-500"
                      />
                      <div className="px-3 py-1 bg-white/10 rounded-lg border border-white/10 text-[10px] font-black text-white uppercase tracking-widest relative z-10">
                        Payable
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className={`flex-[2] py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-100 hover:from-blue-700 hover:to-indigo-700 transition-all flex items-center justify-center gap-2 group disabled:opacity-70`}
                >
                  {submitting ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {feeSummary && (
                <div className="grid grid-cols-3 gap-3">
                  {[
                    [
                      "Total Fees",
                      `₹${feeSummary.admissionDetails.totalFees?.toLocaleString()}`,
                    ],
                    [
                      "Paid",
                      `₹${feeSummary.paymentSummary.totalPaid?.toLocaleString()}`,
                    ],
                    [
                      "Remaining",
                      `₹${feeSummary.paymentSummary.remainingBalance?.toLocaleString()}`,
                    ],
                  ].map(([label, val]) => (
                    <div
                      key={label}
                    ...
                    {feeSummary.installments.map((i: any) => (
                      ...
                      >
                        <span className="text-sm font-medium text-gray-700">
                          Installment {i.installmentNo} — {i.paymentMode}
                        </span>
              ...
              {viewAdm.status === "Active" &&
                feeSummary &&
                !feeSummary.paymentSummary.isFullyPaid && (
                  <div className="border border-dashed border-primary-300 rounded-xl p-4 bg-primary-50/40">
                    <h4 className="text-sm font-bold text-primary-800 mb-3 flex items-center gap-2">
                      <BadgeDollarSign size={16} /> Record Payment
                    </h4>
                    <form onSubmit={handlePayFee} className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            value={feeForm.installmentNo}
                            onChange={(e) =>
                              setFeeForm((f) => ({
                                ...f,
                                installmentNo: +e.target.value as any,
                              }))
                            }
                            className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none bg-white text-sm"
                          >
              ...
              {feeSummary?.paymentSummary?.isFullyPaid &&
                viewAdm.status === "Active" && (
                  <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between">
                    ...
                    <button
                      onClick={() => handleComplete(viewAdm._id)}
              ...
              <div className="px-6 py-5 space-y-4">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setEditForm((f) => ({
                          ...f,
                          totalFees: v,
                          finalPayable: String(+v - +(f.discount || 0)),
                        }));
                      }}
                      className={inputClass}
                    />
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setEditForm((f) => ({
                          ...f,
                          discount: v,
                          finalPayable: String(+(f.totalFees || 0) - +v),
                        }));
                      }}
                      className={inputClass}
                    />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={editForm.finalPayable}
                    onChange={(e) =>
                      setEditForm((f) => ({
                        ...f,
                        finalPayable: e.target.value,
                      }))
                    }
                    className={inputClass}
                  />
