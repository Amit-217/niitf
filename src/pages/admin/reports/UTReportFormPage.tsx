...
const SelectWithCustom: React.FC<SelectWithCustomProps> = ({
  id, value, onChange, customValue, onCustomChange, options, placeholder = 'Select...',
}) => (
...
const emptyObs = (): ObsRow => ({
  ...
  quantity: '', evaluation: '', remark: '',
});

const emptySearchUnit = (): UTSearchUnit => ({
  model: '', angle: '', srNo: '', crystalSize: '', waveMode: '', frequency: '',
});

interface CalibRow { range: string; point1: string; point2: string; point3: string; refDb: string; }
const emptyCalib = (): CalibRow => ({ range: '', point1: '', point2: '', point3: '', refDb: '' });
...
export const UTReportFormPage: React.FC = () => {
  ...
  const [searchUnits, setSearchUnits] = useState<UTSearchUnit[]>([emptySearchUnit()]);
  ...
  const updateUnit = (idx: number, key: keyof UTSearchUnit, val: string) =>
    setSearchUnits(prev => prev.map((row, i) => i === idx ? { ...row, [key]: val } : row));

  const addUnit = () => setSearchUnits(prev => [...prev, emptySearchUnit()]);
  const removeUnit = (idx: number) => setSearchUnits(prev => prev.filter((_, i) => i !== idx));

  const updateObs = (idx: number, key: keyof ObsRow, val: string) =>
    setObservations(prev => prev.map((row, i) => i === idx ? { ...row, [key]: val } : row));

  const addObs = () => setObservations(prev => [...prev, { ...emptyObs(), srNo: prev.length + 1 }]);
  ...
  const handleSubmit = async (status: 'draft' | 'final') => {
    ...
    try {
      const payload = {
        ...
        status,
        jobDetails: {
          customer: jobCustomer, client: jobClient, project: jobProject,
          reportDate: jobReportDate || undefined, inspectionDate: jobInspectionDate || undefined,
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Ultrasonic Testing Report</h1>
      ...
      <div className={sectionClass}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" className={inputClass} defaultValue="FMT-NDT-UT-01" readOnly />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={jobClient} onChange={e => setJobClient(e.target.value)} className={inputClass} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="date" value={jobInspectionEndDate} onChange={e => setJobInspectionEndDate(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Reference Std.</label>
            <SelectWithCustom
              value={jobRefStd} onChange={setJobRefStd}
              customValue={jobRefStdCustom} onCustomChange={setJobRefStdCustom}
              options={['ASME Sec V Article 4', 'ASTM SA 609', 'ASTM SA 435', 'ASTM 578', 'ASTM SA 388', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Inspection Time</label>
            <input type="text" value={jobInspectionTime} onChange={e => setJobInspectionTime(e.target.value)}
              className={inputClass} placeholder="e.g. 02:00 PM to 05:00 PM" />
          </div>
          <div>
            <label className={labelClass}>Acceptance Criteria</label>
            <SelectWithCustom
              value={jobAcceptance} onChange={setJobAcceptance}
              customValue={jobAcceptanceCustom} onCustomChange={setJobAcceptanceCustom}
              options={['ASME SEC VIII Div. 1 Appendix 12', 'ASTM SA 609', 'ASTM SA 435', 'ASTM 578', 'ASTM SA 388', 'Other']}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={jobStage} onChange={e => setJobStage(e.target.value)} className={inputClass}>
          ...
          <div>
            ...
            <input type="text" value={jobThickness} onChange={e => setJobThickness(e.target.value)} className={inputClass} placeholder="e.g. 6,12 & 16 MM" />
          </div>
          <div>
            <label className={labelClass}>Extent of Examination</label>
            <SelectWithCustom
              value={jobExtent} onChange={setJobExtent}
              customValue={jobExtentCustom} onCustomChange={setJobExtentCustom}
              options={['10%', '100%', 'To the maximum extent possible', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Surface Condition</label>
            <select value={jobSurface} onChange={e => setJobSurface(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <select value={jobJointType} onChange={e => setJobJointType(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
              <option>Butt</option>
          ...
          <div>
            ...
            <select value={jobSurfaceTemp} onChange={e => setJobSurfaceTemp(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <select value={jobWeldingProcess} onChange={e => setJobWeldingProcess(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
              <option>SMAW</option>
              <option>GMAW</option>
              <option>FCAW</option>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <SelectWithCustom
              ...
              customValue={eqTypeCustom} onCustomChange={setEqTypeCustom}
              options={['Einstein-II DGS', 'USM 36', 'Kappawave', 'USM 100', 'Other']}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <SelectWithCustom
              ...
              customValue={eqMakeCustom} onCustomChange={setEqMakeCustom}
              options={['Modsonic', 'Waygate', 'Kappawave', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Calibration Due</label>
            <input type="text" value={eqCalibDue} onChange={e => setEqCalibDue(e.target.value)} className={inputClass} placeholder="e.g. 18-12-2025" />
          </div>
          <div>
            <label className={labelClass}>Couplant</label>
            <select value={eqCouplant} onChange={e => setEqCouplant(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
              <option>Water</option>
          ...
          <div>
            ...
            <select value={eqBasicCalib} onChange={e => setEqBasicCalib(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4">
          <h2 className={sectionTitleClass.replace(' mb-4 pb-2 border-b border-gray-100', '')}>Search Unit Details</h2>
              ...
              {searchUnits.map((unit, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={unit.model} onChange={e => updateUnit(idx, 'model', e.target.value)} className={inputClass} placeholder="e.g. Modsonic" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={unit.angle} onChange={e => updateUnit(idx, 'angle', e.target.value)} className={inputClass}>
                      <option>45Â°</option>
                      <option>60Â°</option>
                      <option>70Â°</option>
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={unit.srNo} onChange={e => updateUnit(idx, 'srNo', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={unit.crystalSize} onChange={e => updateUnit(idx, 'crystalSize', e.target.value)} className={inputClass}>
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={unit.waveMode} onChange={e => updateUnit(idx, 'waveMode', e.target.value)} className={inputClass}>
                      <option>Shear</option>
                      <option>Longitudinal</option>
                    </select>
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={unit.frequency} onChange={e => updateUnit(idx, 'frequency', e.target.value)} className={inputClass}>
                  ...
                  <td className="border border-gray-200 px-1 py-1 text-center">
                    {searchUnits.length > 1 && (
                      <button type="button" onClick={() => removeUnit(idx)} className="text-red-400 hover:text-red-600">
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select value={utMethod} onChange={e => setUtMethod(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <SelectWithCustom
              value={refCalibBlock} onChange={setRefCalibBlock}
              customValue={refCalibBlockCustom} onCustomChange={setRefCalibBlockCustom}
              options={['19 mm', '38 mm', 'Job itself', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>UT Calibration Method</label>
            <select value={utCalibMethod} onChange={e => setUtCalibMethod(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <input type="text" value={scanningDb} onChange={e => setScanningDb(e.target.value)}
              className={inputClass} placeholder="e.g. Ref dB+6dB" />
          </div>
          <div className="sm:col-span-2">
            <label className={labelClass}>Scanning Sensitivity</label>
            <SelectWithCustom
              value={scanningSens} onChange={setScanningSens}
              customValue={scanningSensCustom} onCustomChange={setScanningSensCustom}
              options={['Ã˜ 2.5 mm SDH', 'Ã˜ 3mm SDH', '1" BWE set @ 80% of FSH on Job', 'Other']}
            />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-600 uppercase">
                <th className="border border-gray-200 px-3 py-2 text-left w-28">Point</th>
                {calibAngles.map(a => (
                  <th key={a.label} className="border border-gray-200 px-2 py-2 text-center">{a.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              ] as { key: keyof CalibRow; label: string }[]).map(row => (
                <tr key={row.key} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-3 py-2 font-medium text-gray-600 bg-gray-50">{row.label}</td>
                  {calibAngles.map(a => (
                    <td key={a.label} className="border border-gray-200 px-1 py-1">
                      <input type="text" value={a.state[row.key]}
                        onChange={e => updateCalib(a.setter, row.key, e.target.value)}
          ...
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <input type="text" value={inspectorQual} onChange={e => setInspectorQual(e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>I.D. No.</label>
                <input type="text" value={inspectorIdNo} onChange={e => setInspectorIdNo(e.target.value)} className={inputClass} />
              </div>
              <div>
