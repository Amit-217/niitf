...
const SelectWithCustom: React.FC<SelectWithCustomProps> = ({
  value, onChange, customValue, onCustomChange, options, placeholder = 'Select...',
}) => (
  <div className="space-y-1">
    ...
    {value === 'Other' && (
      <input type="text" value={customValue} onChange={e => onCustomChange(e.target.value)}
        placeholder="Specify other value..." className={inputClass} />
    )}
  </div>
);
...
interface SkipRow { bp: string; mm: string; fsh: string; }
const emptySkipRow = (): SkipRow => ({ bp: '', mm: '', fsh: '' });

interface ProbeModeFormData {
  half: SkipRow; one: SkipRow; oneHalf: SkipRow; two: SkipRow;
  dacDb: string; scanningDb: string;
}
const emptyPMD = (): ProbeModeFormData => ({
  half: emptySkipRow(), one: emptySkipRow(), oneHalf: emptySkipRow(), two: emptySkipRow(),
  dacDb: '', scanningDb: '',
});
const emptyCalibTable = (): Record<ProbeMode, ProbeModeFormData> => ({
  '45L': emptyPMD(), '45T': emptyPMD(),
  '60L': emptyPMD(), '60T': emptyPMD(),
  '70L': emptyPMD(), '70T': emptyPMD(),
});

const SKIPS: Array<{ key: keyof Pick<ProbeModeFormData, 'half' | 'one' | 'oneHalf' | 'two'>; label: string }> = [
  { key: 'half', label: '½' },
  { key: 'one', label: '1' },
  { key: 'oneHalf', label: '1½' },
  { key: 'two', label: '2' },
...
export const VSSCUTReportFormPage: React.FC = () => {
  ...
  const [apcFrequency, setApcFrequency] = useState('4 MHz');
  const [apcFrequencyCustom, setApcFrequencyCustom] = useState('');
  const [apcSize, setApcSize] = useState('8x9 mm');
  const [apcSizeCustom, setApcSizeCustom] = useState('');
  const [apcType, setApcType] = useState('MWB');
  ...
  const [calibTable, setCalibTable] = useState<Record<ProbeMode, ProbeModeFormData>>(emptyCalibTable());

  // ── Normal Probe Calibration ──
  const [npProbeType, setNpProbeType] = useState('');
  const [npFrequency, setNpFrequency] = useState('4 MHz');
  ...
  const updateCalibSingle = (mode: ProbeMode, field: 'dacDb' | 'scanningDb', val: string) =>
    setCalibTable(prev => ({ ...prev, [mode]: { ...prev[mode], [field]: val } }));
  ...
  const handleSubmit = async (status: 'draft' | 'final') => {
    ...
    try {
      ...
      PROBE_MODES.forEach(pm => { ct[pm] = calibTable[pm] as VSSCUTProbeModeData; });
      ...
      await createVSSCUTReport({
        ...
        acceptanceStandard: resolve(acceptanceStandard, acceptanceStandardCustom),
        referenceDatum: resolve(referenceDatum, referenceDatumCustom),
        testSetup: {
          angleRange: tsAngleRange,
          normalRange: tsNormalRange,
          standardCalBlock: { angle: tsCalBlockAngle, normal: tsCalBlockNormal },
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">VSSC Ultrasonic Test Report</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" value={reportNo} onChange={e => setReportNo(e.target.value)}
              className={inputClass} placeholder="e.g. NIIT/WIL/UT/278" />
          </div>
          <div>
            <label className={labelClass}>Page No.</label>
            <input type="text" value={pageNo} onChange={e => setPageNo(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Format No.</label>
            <input type="text" className={inputClass} defaultValue="FMT-NDT-VSSC-UT-01" readOnly />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <input type="text" value={jobDescription} onChange={e => setJobDescription(e.target.value)}
              className={inputClass} placeholder="e.g. 783107 HS200 NOZZLE END SEGMENT [HS200-NES-20-WIL]" />
          </div>
          <div>
            <label className={labelClass}>Customer</label>
            <input type="text" value={customerName} disabled className={inputClass + ' bg-gray-100 cursor-not-allowed'} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={weldJointNo} onChange={e => setWeldJointNo(e.target.value)}
              className={inputClass} placeholder="e.g. As per Annexure I" />
          </div>
          <div>
            <label className={labelClass}>Thickness of Job</label>
            <input type="text" value={thicknessOfJob} onChange={e => setThicknessOfJob(e.target.value)}
              className={inputClass} placeholder="e.g. 7.8MM / 7.7MM / 12MM / 18MM" />
          </div>
          <div>
            <label className={labelClass}>Surface Condition</label>
            <SelectWithCustom
              value={surfaceCondition} onChange={setSurfaceCondition}
              customValue={surfaceConditionCustom} onCustomChange={setSurfaceConditionCustom}
          ...
          <div>
            ...
            <input type="text" value={material} onChange={e => setMaterial(e.target.value)}
              className={inputClass} placeholder="e.g. MDN 250" />
          </div>
          <div>
          ...
          <div>
            ...
            <SelectWithCustom
              value={scanningTechnique} onChange={setScanningTechnique}
              customValue={scanningTechniqueCustom} onCustomChange={setScanningTechniqueCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={stageOfInspection} onChange={setStageOfInspection}
              customValue={stageOfInspectionCustom} onCustomChange={setStageOfInspectionCustom}
              options={['Before HT', 'AA Periodic UT', 'After Ageing', 'After PPT', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Equipment Used</label>
            <SelectWithCustom
              value={equipmentUsed} onChange={setEquipmentUsed}
              customValue={equipmentUsedCustom} onCustomChange={setEquipmentUsedCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={couplant} onChange={setCouplant}
              customValue={couplantCustom} onCustomChange={setCouplantCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={areaScanned} onChange={setAreaScanned}
              customValue={areaScannedCustom} onCustomChange={setAreaScannedCustom}
              options={['Weld + HAZ (Longitudinal & Transverse – 2 Directions)', 'Weld Only', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Acceptance Standard</label>
            <SelectWithCustom
              value={acceptanceStandard} onChange={setAcceptanceStandard}
              customValue={acceptanceStandardCustom} onCustomChange={setAcceptanceStandardCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={referenceDatum} onChange={setReferenceDatum}
              customValue={referenceDatumCustom} onCustomChange={setReferenceDatumCustom}
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={tsAngleRange} onChange={e => setTsAngleRange(e.target.value)}
              className={inputClass} placeholder="e.g. 0-100 mm" />
          </div>
          <div>
            <label className={labelClass}>For Normal — Range</label>
            <input type="text" value={tsNormalRange} onChange={e => setTsNormalRange(e.target.value)}
              className={inputClass} placeholder="e.g. 0-10 mm" />
          </div>
          <div>
            <label className={labelClass}>Standard Cal Block — For Angle</label>
            <input type="text" value={tsCalBlockAngle} onChange={e => setTsCalBlockAngle(e.target.value)}
              className={inputClass} placeholder="e.g. V-2 Block (MDN-250)" />
          </div>
          <div>
            <label className={labelClass}>Standard Cal Block — For Normal</label>
            <input type="text" value={tsCalBlockNormal} onChange={e => setTsCalBlockNormal(e.target.value)}
              className={inputClass} placeholder="e.g. Step Block (MDN 250)" />
          </div>
          <div>
            <label className={labelClass}>Idtn. No of Ref Block — For Angle</label>
            <input type="text" value={tsRefBlockAngle} onChange={e => setTsRefBlockAngle(e.target.value)}
              className={inputClass} placeholder="e.g. 'G' Notch (LSP 34 A side only)" />
          </div>
          <div>
            <label className={labelClass}>Idtn. No of Ref Block — For Normal</label>
            <SelectWithCustom
              value={tsRefBlockNormal} onChange={setTsRefBlockNormal}
              customValue={tsRefBlockNormalCustom} onCustomChange={setTsRefBlockNormalCustom}
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          <div>
            ...
            <SelectWithCustom
              value={apcFrequency} onChange={setApcFrequency}
              customValue={apcFrequencyCustom} onCustomChange={setApcFrequencyCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={apcSize} onChange={setApcSize}
              customValue={apcSizeCustom} onCustomChange={setApcSizeCustom}
          ...
          <div>
            ...
            <SelectWithCustom
              value={apcType} onChange={setApcType}
              customValue={apcTypeCustom} onCustomChange={setApcTypeCustom}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          <div>
            ...
            <input type="text" value={probe45Sr} onChange={e => setProbe45Sr(e.target.value)}
              className={inputClass} placeholder="e.g. 63230" />
          </div>
          <div>
            <label className={labelClass}>60° Probe Sr. No.</label>
            <input type="text" value={probe60Sr} onChange={e => setProbe60Sr(e.target.value)}
              className={inputClass} placeholder="e.g. 63285" />
          </div>
          <div>
            <label className={labelClass}>70° Probe Sr. No.</label>
            <input type="text" value={probe70Sr} onChange={e => setProbe70Sr(e.target.value)}
              className={inputClass} placeholder="e.g. 63309" />
          </div>
        </div>
          ...
          <table className="w-full text-xs border-collapse min-w-[860px]">
            <thead>
              <tr className="bg-gray-800 text-white text-center">
                ...
                {PROBE_MODES.map(pm => (
                  <th key={pm} className="border border-gray-400 px-1 py-2 font-bold" colSpan={3}>{pm}</th>
                ))}
              </tr>
            ...
            <tbody>
              {SKIPS.map(({ key, label }) => (
                <tr key={key} className="hover:bg-gray-50">
                  <td className="border border-gray-300 px-2 py-1 text-center font-semibold text-gray-700 bg-gray-50">{label}</td>
                  {PROBE_MODES.map(pm => (
                    <React.Fragment key={pm}>
                      {(['bp', 'mm', 'fsh'] as const).map(field => (
                        <td key={field} className="border border-gray-300 p-0">
                          <input
                            ...
                            onChange={e => updateCalibCell(pm, key, field, e.target.value)}
              ...
              <tr className="bg-amber-50">
                <td className="border border-gray-300 px-2 py-1.5 text-center text-[11px] font-bold text-gray-700 whitespace-nowrap">DAC dB</td>
                {PROBE_MODES.map(pm => (
                  <td key={pm} className="border border-gray-300 p-0" colSpan={3}>
                    <input
                      type="text"
                      value={calibTable[pm].dacDb}
                      onChange={e => updateCalibSingle(pm, 'dacDb', e.target.value)}
              ...
              <tr className="bg-amber-50">
                <td className="border border-gray-300 px-2 py-1.5 text-center text-[11px] font-bold text-gray-700 whitespace-nowrap">Scan dB</td>
                {PROBE_MODES.map(pm => (
                  <td key={pm} className="border border-gray-300 p-0" colSpan={3}>
                    <input
                      type="text"
                      value={calibTable[pm].scanningDb}
                      onChange={e => updateCalibSingle(pm, 'scanningDb', e.target.value)}
                      className="w-full border-0 text-xs px-1 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-400 rounded text-center bg-transparent"
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={npProbeType} onChange={e => setNpProbeType(e.target.value)}
              className={inputClass} placeholder="e.g. 75798 / MSEB 4E" />
          </div>
          <div>
            <label className={labelClass}>Frequency</label>
            <SelectWithCustom
              value={npFrequency} onChange={setNpFrequency}
              customValue={npFrequencyCustom} onCustomChange={setNpFrequencyCustom}
              options={['4 MHz', '2 MHz', '5 MHz', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Size</label>
            <input type="text" value={npSize} onChange={e => setNpSize(e.target.value)}
              className={inputClass} placeholder="e.g. 10 mm (TR)" />
          </div>
          <div>
            <label className={labelClass}>Skip</label>
            <input type="text" value={npSkip} onChange={e => setNpSkip(e.target.value)}
              className={inputClass} placeholder="e.g. 4.0" />
          </div>
          <div>
            <label className={labelClass}>BP – %FSH</label>
            <input type="text" value={npBp} onChange={e => setNpBp(e.target.value)}
              className={inputClass} placeholder="e.g. 80 %" />
          </div>
          <div>
            <label className={labelClass}>DAC dB</label>
            <input type="text" value={npDacDb} onChange={e => setNpDacDb(e.target.value)}
              className={inputClass} placeholder="e.g. 49.0 dB" />
          </div>
          <div>
            <label className={labelClass}>Scanning dB</label>
            <input type="text" value={npScanningDb} onChange={e => setNpScanningDb(e.target.value)}
              className={inputClass} placeholder="e.g. 49.0 + 10 dB" />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <SelectWithCustom
              ...
              customValue={remarksCustom} onCustomChange={setRemarksCustom}
              options={[
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            ...
            <div className="space-y-2">
              <div><label className={labelClass}>Inspector Name</label>
                <input type="text" value={inspectorName} onChange={e => setInspectorName(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Qualification</label>
                <input type="text" value={inspectorQual} onChange={e => setInspectorQual(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>I.D. No.</label>
                <input type="text" value={inspectorIdNo} onChange={e => setInspectorIdNo(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Date</label>
                <input type="date" value={inspectorDate} onChange={e => setInspectorDate(e.target.value)} className={inputClass} /></div>
            </div>
          </div>
          {/* QC */}
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">QC / WIL</p>
            <div className="space-y-2">
              <div><label className={labelClass}>Name</label>
                <input type="text" value={qcName} onChange={e => setQcName(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>I.D. No.</label>
                <input type="text" value={qcIdNo} onChange={e => setQcIdNo(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Date</label>
                <input type="date" value={qcDate} onChange={e => setQcDate(e.target.value)} className={inputClass} /></div>
            </div>
          </div>
          {/* RQS */}
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">RQS / VSSC</p>
            <div className="space-y-2">
              <div><label className={labelClass}>Name</label>
                <input type="text" value={rqsName} onChange={e => setRqsName(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>I.D. No.</label>
                <input type="text" value={rqsIdNo} onChange={e => setRqsIdNo(e.target.value)} className={inputClass} /></div>
              <div><label className={labelClass}>Date</label>
                <input type="date" value={rqsDate} onChange={e => setRqsDate(e.target.value)} className={inputClass} /></div>
            </div>
          </div>
        </div>
      </div>
