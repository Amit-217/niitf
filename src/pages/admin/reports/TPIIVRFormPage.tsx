...
const SelectWithOther: React.FC<SelectWithOtherProps> = ({
  ...
  <div className="space-y-1">
    ...
    {value === 'Other' && (
      <input type="text" value={otherValue} onChange={e => onOtherChange(e.target.value)} placeholder="Specify other value..." className={inputClass} />
    )}
  </div>
);
...
const emptyItem = (): InspectionItemRow => ({
  poLineNo: '', description: '', drgOrHeatNo: '', qtyOffered: '', qtyInspected: '',
...
export const TPIIVRFormPage: React.FC = () => {
  ...
  const removeCalib = (idx: number) => setCalibRows(prev => prev.filter((_, i) => i !== idx));
  ...
  const handleSubmit = async (status: 'draft' | 'final') => {
    ...
    try {
      const payload = {
        ...
        inspectionItems: items.filter(i => i.description.trim()).map(i => ({
          poLineNo: i.poLineNo,
        ...
        conclusion: resolve(conclusion, conclusionOther),
        referenceDocuments: refDocs.map(d => ({
          document: d.document,
          referenceNumber: d.referenceNumber,
          revNo: d.revNo,
        })),
        calibrationStatus: calibRows.filter(c => c.equipment.trim()).map(c => ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Inspection Visit Report</h1>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={inspectionLocation} onChange={e => setInspectionLocation(e.target.value)} className={inputClass} placeholder="e.g. M/s Bharat Forge Ltd. Pune." />
          </div>
          <div>
            <label className={labelClass}>Project</label>
            <input type="text" value={project} onChange={e => setProject(e.target.value)} className={inputClass} placeholder="e.g. --" />
          </div>
          <div>
            <label className={labelClass}>Appd. QAP No.</label>
            <input type="text" value={appdQapNo} onChange={e => setAppdQapNo(e.target.value)} className={inputClass} placeholder="e.g. QAP/METSO/D56263 Rev.00" />
          </div>
          <div>
            <label className={labelClass}>Client PO No.</label>
            <input type="text" value={clientPoNo} onChange={e => setClientPoNo(e.target.value)} className={inputClass} placeholder="e.g. 7500188455" />
          </div>
          <div>
            <label className={labelClass}>Appd. QAP Date</label>
            <input type="date" value={appdQapDt} onChange={e => setAppdQapDt(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>PO Amed. No.</label>
            <input type="text" value={poAmedNo} onChange={e => setPoAmedNo(e.target.value)} className={inputClass} placeholder="e.g. ---" />
          </div>
          <div>
            <label className={labelClass}>Part Name</label>
            <input type="text" value={partName} onChange={e => setPartName(e.target.value)} className={inputClass} placeholder="e.g. SHAFT STEDIMENT" />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <SelectWithOther
              ...
              otherValue={inspectionStageOther} onOtherChange={setInspectionStageOther}
              options={['UT IN P/M CONDITION', 'STAGE', 'FINAL', 'STAGE & FINAL', 'INCOMING', 'IN-PROCESS', 'DISPATCH', 'Other']}
            />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">Client Details</p>
            <div className="space-y-2">
              <div>
                <label className={labelClass}>Ref</label>
                <SelectWithOther
                  value={clientRef} onChange={setClientRef}
                  otherValue={clientRefOther} onOtherChange={setClientRefOther}
                  options={['By Mail', 'By Phone', 'By Email', 'By Fax', 'Other']}
                />
              </div>
              <div>
                <label className={labelClass}>Contact</label>
                <input type="text" value={clientContact} onChange={e => setClientContact(e.target.value)} className={inputClass} placeholder="e.g. Mr. Abdul Najmi" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={inspAttDt} onChange={e => setInspAttDt(e.target.value)} className={inputClass} />
              </div>
            </div>
          </div>
          {/* Vendor Details */}
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">Vendor Details</p>
            <div className="space-y-2">
              <div>
                <label className={labelClass}>Vendor</label>
                <input type="text" value={vendor} onChange={e => setVendor(e.target.value)} className={inputClass} placeholder="e.g. Bharat Forge Ltd. Mundhwa, Pune." />
              </div>
              <div>
                <label className={labelClass}>Sub Vendor</label>
                <input type="text" value={subVendor} onChange={e => setSubVendor(e.target.value)} className={inputClass} placeholder="e.g. --" />
              </div>
              <div>
                <label className={labelClass}>Contact</label>
                <input type="text" value={vendorContact} onChange={e => setVendorContact(e.target.value)} className={inputClass} placeholder="e.g. Mr. Sanskar Karav" />
              </div>
              <div>
                <label className={labelClass}>Phone</label>
                <input type="text" value={vendorPhone} onChange={e => setVendorPhone(e.target.value)} className={inputClass} placeholder="e.g. +91 8483024812" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
          <div>
            ...
            <input type="text" value={extraVisitDate} onChange={e => setExtraVisitDate(e.target.value)} className={inputClass} placeholder="e.g. NA" />
          </div>
          <div>
            <label className={labelClass}>Comment</label>
            <input type="text" value={extraVisitComment} onChange={e => setExtraVisitComment(e.target.value)} className={inputClass} placeholder="e.g. NA" />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-indigo-700 uppercase tracking-wide">Inspection Items</h2>
          <button type="button" onClick={addItem} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition-colors">
          ...
          <table className="w-full text-sm border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-600 uppercase">
                <th className="border border-gray-200 px-2 py-2 text-left">PO Line No.</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Description</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Drg No. / Heat No.</th>
                <th className="border border-gray-200 px-2 py-2 text-center">Offered</th>
                <th className="border border-gray-200 px-2 py-2 text-center">Inspected</th>
                <th className="border border-gray-200 px-2 py-2 text-center">Accepted</th>
                <th className="border border-gray-200 px-2 py-2 text-center">Hold</th>
                <th className="border border-gray-200 px-2 py-2 text-center">Reject</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Insp. Type</th>
              ...
              {items.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.poLineNo} onChange={e => updateItem(idx, 'poLineNo', e.target.value)} className={inputClass} placeholder="01" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.description} onChange={e => updateItem(idx, 'description', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.drgOrHeatNo} onChange={e => updateItem(idx, 'drgOrHeatNo', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="number" value={row.qtyOffered} onChange={e => updateItem(idx, 'qtyOffered', e.target.value)} className={inputClass + ' w-16'} min="0" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="number" value={row.qtyInspected} onChange={e => updateItem(idx, 'qtyInspected', e.target.value)} className={inputClass + ' w-16'} min="0" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="number" value={row.qtyAccepted} onChange={e => updateItem(idx, 'qtyAccepted', e.target.value)} className={inputClass + ' w-16'} min="0" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="number" value={row.qtyHold} onChange={e => updateItem(idx, 'qtyHold', e.target.value)} className={inputClass + ' w-16'} min="0" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="number" value={row.qtyReject} onChange={e => updateItem(idx, 'qtyReject', e.target.value)} className={inputClass + ' w-16'} min="0" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={row.inspectionType} onChange={e => updateItem(idx, 'inspectionType', e.target.value)} className={inputClass}>
      ...
      {/* ── Inspection Activities & Conclusion ── */}
      <div className={sectionClass}>
        <h2 className={sectionTitleClass}>Inspection Activities &amp; Conclusion</h2>
        <div className="space-y-4">
          ...
          <div>
            ...
            <SelectWithOther
              ...
              otherValue={conclusionOther} onOtherChange={setConclusionOther}
              options={[
                'Ultrasonic Testing witness done & found accepted.',
                'Dimensional Inspection done & found accepted.',
                'Visual Inspection done & found accepted.',
                'Inspection done & found accepted.',
                'Inspection done & found rejected.',
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-indigo-700 uppercase tracking-wide">Calibration Status of Instruments</h2>
          <button type="button" onClick={addCalib} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition-colors">
            <Plus className="w-3 h-3" /> Add Row
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-600 uppercase">
                <th className="border border-gray-200 px-2 py-2 text-left">Equipment / Instrument</th>
                <th className="border border-gray-200 px-2 py-2 text-left">I.D. Number</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Calibration Date</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Due Date</th>
                <th className="border border-gray-200 px-2 py-2 text-left">NABL Certified</th>
                <th className="border border-gray-200 px-2 py-2 w-8"></th>
              </tr>
            </thead>
            <tbody>
              {calibRows.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.equipment} onChange={e => updateCalib(idx, 'equipment', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.idNumber} onChange={e => updateCalib(idx, 'idNumber', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="date" value={row.calibrationDate} onChange={e => updateCalib(idx, 'calibrationDate', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="date" value={row.dueDate} onChange={e => updateCalib(idx, 'dueDate', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={row.nablCertified} onChange={e => updateCalib(idx, 'nablCertified', e.target.value)} className={inputClass}>
                      <option value="">Select...</option>
                      <option>Yes</option>
                      <option>No</option>
                    </select>
                  ...
                  <td className="border border-gray-200 px-1 py-1 text-center">
                    {calibRows.length > 1 && (
                      <button type="button" onClick={() => removeCalib(idx)} className="text-red-400 hover:text-red-600">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    )}
                  </td>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {/* For Vendor */}
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">For Vendor</p>
            <div className="space-y-2">
              <div>
                <label className={labelClass}>Vendor Name</label>
                <input type="text" value={vendorSignName} onChange={e => setVendorSignName(e.target.value)} className={inputClass} placeholder="e.g. Mr. Ravindra Naral" />
              </div>
              <div>
                <label className={labelClass}>Date</label>
                <input type="date" value={vendorSignDate} onChange={e => setVendorSignDate(e.target.value)} className={inputClass} />
              </div>
            </div>
          </div>
          {/* For NIIT Surveyor */}
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            <p className="text-xs font-semibold text-gray-600 uppercase mb-3">For NIIT Surveyor, Baramati</p>
            <div className="space-y-2">
              <div>
                <label className={labelClass}>Name</label>
                <input type="text" value={niitSignName} onChange={e => setNiitSignName(e.target.value)} className={inputClass} placeholder="e.g. Mr. Suraj Dubal" />
              </div>
              <div>
                <label className={labelClass}>Date</label>
                <input type="date" value={niitSignDate} onChange={e => setNiitSignDate(e.target.value)} className={inputClass} />
              </div>
            </div>
          </div>
        </div>
