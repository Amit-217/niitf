...
export const PTReportFormPage: React.FC = () => {
  ...
  const handleSubmit = async (status: 'draft' | 'final') => {
    ...
    try {
      await createPTReport({
        ...
        consumablesDetails: {
          penetrant: { manufacturer: resolve(penMfr, penMfrCustom), batch: penBatch, expiryDate: penExpiry },
          developer: { manufacturer: resolve(devMfr, devMfrCustom), batch: devBatch, expiryDate: devExpiry },
          cleaner: { manufacturer: resolve(cleanMfr, cleanMfrCustom), batch: cleanBatch, expiryDate: cleanExpiry },
        },
        methodDescription: {
          dwellTime, lightIntensity, developingTime,
          lightEquipmentUsed: resolve(lightEquip, lightEquipCustom),
          postCleaning, dryingTime,
        },
  ...
  const mfrOptions = ['Pradeep', 'Dyeglo', 'Ferrochem', 'MR Chem', 'Magnaflux', 'Other'];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Liquid Penetrant Test Report</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={reportNo} onChange={e => setReportNo(e.target.value)}
              className={inputClass} placeholder="e.g. NIIT/GE/PT/25-26/01" />
          </div>
          <div>
            <label className={labelClass}>Format No.</label>
            <input type="text" className={inputClass} defaultValue="FMT-NDT-PT-01" readOnly />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={jobProject} onChange={e => setJobProject(e.target.value)} className={inputClass} placeholder="e.g. OIL COOLER FOR GEAR BOX" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={jobInspectionTime} onChange={e => setJobInspectionTime(e.target.value)}
              className={inputClass} placeholder="e.g. 10:00 AM â€“ 03:30 PM" />
          </div>
          <div>
            ...
            <SelectWithCustom
              ...
              options={['ASME Sec. VIII Div. 1, Appendix 7', 'Appendix 8', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Material</label>
            <input type="text" value={jobMaterial} onChange={e => setJobMaterial(e.target.value)} className={inputClass} placeholder="e.g. IS 2062 E250 BR" />
          </div>
          <div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select value={penetrantMethod} onChange={e => setPenetrantMethod(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <div>
            ...
            <select value={removalMethod} onChange={e => setRemovalMethod(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50">
                <th className="border border-gray-200 px-3 py-2 text-left font-medium text-gray-700 w-28">Material</th>
                <th className="border border-gray-200 px-3 py-2 text-left font-medium text-gray-700">Manufacture</th>
            ...
            <tbody>
              {[
                { label: 'Penetrant', mfr: penMfr, setMfr: setPenMfr, mfrC: penMfrCustom, setMfrC: setPenMfrCustom, batch: penBatch, setBatch: setPenBatch, expiry: penExpiry, setExpiry: setPenExpiry },
                { label: 'Developer', mfr: devMfr, setMfr: setDevMfr, mfrC: devMfrCustom, setMfrC: setDevMfrCustom, batch: devBatch, setBatch: setDevBatch, expiry: devExpiry, setExpiry: setDevExpiry },
                { label: 'Cleaner', mfr: cleanMfr, setMfr: setCleanMfr, mfrC: cleanMfrCustom, setMfrC: setCleanMfrCustom, batch: cleanBatch, setBatch: setCleanBatch, expiry: cleanExpiry, setExpiry: setCleanExpiry },
              ].map(row => (
                <tr key={row.label}>
                  <td className="border border-gray-200 px-3 py-2 font-medium text-gray-600">{row.label}</td>
                  <td className="border border-gray-200 px-2 py-1">
                    <SelectWithCustom value={row.mfr} onChange={row.setMfr} customValue={row.mfrC} onCustomChange={row.setMfrC} options={mfrOptions} />
                  </td>
                  <td className="border border-gray-200 px-2 py-1">
                    <input type="text" value={row.batch} onChange={e => row.setBatch(e.target.value)} className={inputClass} placeholder="Batch No." />
                  </td>
                  <td className="border border-gray-200 px-2 py-1">
                    <input type="text" value={row.expiry} onChange={e => row.setExpiry(e.target.value)} className={inputClass} placeholder="e.g. 08/26" />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={dwellTime} onChange={e => setDwellTime(e.target.value)} className={inputClass} placeholder="e.g. 7 to 10 Min" />
          </div>
          <div>
            <label className={labelClass}>Light Intensity</label>
            <input type="text" value={lightIntensity} onChange={e => setLightIntensity(e.target.value)} className={inputClass} placeholder="e.g. 1250 lux" />
          </div>
          <div>
            <label className={labelClass}>Developing Time</label>
            <input type="text" value={developingTime} onChange={e => setDevelopingTime(e.target.value)} className={inputClass} placeholder="e.g. 15-20 Min" />
          </div>
          <div>
            <label className={labelClass}>Light Equipment Used</label>
            <SelectWithCustom
              value={lightEquip} onChange={setLightEquip}
              customValue={lightEquipCustom} onCustomChange={setLightEquipCustom}
              options={['60 W Bulb', 'NA', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Post Cleaning</label>
            <select value={postCleaning} onChange={e => setPostCleaning(e.target.value)} className={inputClass}>
              <option value="">Select...</option>
              <option>Done</option>
          ...
          <div>
            ...
            <input type="text" value={dryingTime} onChange={e => setDryingTime(e.target.value)} className={inputClass} placeholder="e.g. 10 Min / NA" />
          </div>
        </div>
      </div>
