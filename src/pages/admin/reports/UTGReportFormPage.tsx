...
const emptySearchUnit = (): SearchUnitRow => ({
  searchUnit: '', model: '', angle: '', srNo: '', crystalSize: '', crystalSizeOther: '',
  waveMode: '', frequency: '', frequencyOther: '',
});

const emptyTechnique = (): TechniqueRow => ({
  searchUnit: '', angle: '', srNo: '', crystalSize: '', crystalSizeOther: '',
  waveMode: '', frequency: '', frequencyOther: '',
});

const emptyObs = (): ObsRow => ({ srNo: 1, itemName: '', measuredThickness: '', remark: '' });
...
export const UTGReportFormPage: React.FC = () => {
  ...
  // ── Search Unit Details ──
  const [searchUnits, setSearchUnits] = useState<SearchUnitRow[]>([emptySearchUnit()]);

  // ── Technique Details ──
  const [techniques, setTechniques] = useState<TechniqueRow[]>([emptyTechnique()]);
  ...
  const updateSearchUnit = (idx: number, key: keyof SearchUnitRow, val: string) =>
    setSearchUnits(prev => prev.map((r, i) => i === idx ? { ...r, [key]: val } : r));
  const addSearchUnit = () => setSearchUnits(prev => [...prev, emptySearchUnit()]);
  ...
  const updateTechnique = (idx: number, key: keyof TechniqueRow, val: string) =>
    setTechniques(prev => prev.map((r, i) => i === idx ? { ...r, [key]: val } : r));
  const addTechnique = () => setTechniques(prev => [...prev, emptyTechnique()]);
  ...
  const updateObs = (idx: number, key: keyof ObsRow, val: string) =>
    setObservations(prev => prev.map((r, i) => i === idx ? { ...r, [key]: val } : r));
  const addObs = () =>
    ...
    try {
      const payload = {
        ...
        jobDetails: {
          ...
          surfaceCondition: resolve(jobSurfaceCondition, jobSurfaceConditionOther),
        ...
        searchUnitDetails: searchUnits.filter(u => u.searchUnit.trim() || u.model.trim()).map(u => ({
        ...
        techniqueDetails: {
          ...
          techniques: techniques.filter(t => t.searchUnit.trim() || t.angle.trim()).map(t => ({
        ...
        observations: observations.filter(o => o.itemName.trim()).map(o => ({
          srNo: o.srNo,
          itemName: o.itemName,
        ...
        finalSection: {
          ...
          inspector: [{ name: inspectorName, qualification: inspectorQual, signature: inspectorSig, idNo: inspectorIdNo, date: inspectorDate || undefined }],
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Ultrasonic Thickness Gauging Report</h1>
      ...
      <div className={sectionClass}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={reportNo} onChange={e => setReportNo(e.target.value)} className={inputClass} placeholder="e.g. NIIT/VESSPL/UT/25-26/01" />
          </div>
          <div>
            <label className={labelClass}>Format No.</label>
            <input type="text" className={inputClass} defaultValue="FMT-NDT-UTG-01" readOnly />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={jobCustomer} disabled className={inputClass + ' bg-gray-100 cursor-not-allowed'} />
          </div>
          <div>
            <label className={labelClass}>Report No.</label>
          ...
          <div>
            ...
            <input type="text" value={jobClient} onChange={e => setJobClient(e.target.value)} className={inputClass} placeholder="e.g. MTA AUTOMOTIVE" />
          </div>
          <div>
            <label className={labelClass}>Report Date</label>
            <input type="date" value={jobReportDate} onChange={e => setJobReportDate(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Project</label>
            <input type="text" value={jobProject} onChange={e => setJobProject(e.target.value)} className={inputClass} placeholder="e.g. VC-200412" />
          </div>
          <div>
            <label className={labelClass}>Inspection Date</label>
            <input type="date" value={jobInspectionDate} onChange={e => setJobInspectionDate(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Reference Std.</label>
            <SelectWithOther
              value={jobRefStd} onChange={setJobRefStd}
              otherValue={jobRefStdOther} onOtherChange={setJobRefStdOther}
              options={['ASME Sec V Article 4', 'ASME Sec V Article 5', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Inspection Time</label>
            <input type="text" value={jobInspectionTime} onChange={e => setJobInspectionTime(e.target.value)} className={inputClass} placeholder="e.g. 02:00 PM to 05:00 PM" />
          </div>
          <div>
            <label className={labelClass}>Acceptance Criteria</label>
            <input type="text" value={jobAcceptanceCriteria} onChange={e => setJobAcceptanceCriteria(e.target.value)} className={inputClass} placeholder="e.g. ASME SEC VIII Appendix" />
          </div>
          <div>
            <label className={labelClass}>Material</label>
            <input type="text" value={jobMaterial} onChange={e => setJobMaterial(e.target.value)} className={inputClass} placeholder="e.g. IS 2062 E-250 BR" />
          </div>
          <div>
            <label className={labelClass}>Stage of Inspection</label>
            <SelectWithOther
              value={jobStage} onChange={setJobStage}
              otherValue={jobStageOther} onOtherChange={setJobStageOther}
              options={['After welding', 'As Casting', 'As Rolled', 'As Forged', 'In-service', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Surface Condition</label>
            <SelectWithOther
              value={jobSurfaceCondition} onChange={setJobSurfaceCondition}
              otherValue={jobSurfaceConditionOther} onOtherChange={setJobSurfaceConditionOther}
          ...
          <div>
            ...
            <SelectWithOther
              value={jobExtent} onChange={setJobExtent}
              otherValue={jobExtentOther} onOtherChange={setJobExtentOther}
          ...
          <div>
            ...
            <SelectWithOther
              value={jobSurfaceTemp} onChange={setJobSurfaceTemp}
              otherValue={jobSurfaceTempOther} onOtherChange={setJobSurfaceTempOther}
      ...
      <div className={sectionClass}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <SelectWithOther
              value={eqType} onChange={setEqType}
              otherValue={eqTypeOther} onOtherChange={setEqTypeOther}
              options={['Modsonic', 'EEC', 'Waygate', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Sr. No.</label>
            <input type="text" value={eqSrNo} onChange={e => setEqSrNo(e.target.value)} className={inputClass} placeholder="e.g. E4238-0215" />
          </div>
          <div>
            <label className={labelClass}>Make</label>
            <SelectWithOther
              value={eqMake} onChange={setEqMake}
              otherValue={eqMakeOther} onOtherChange={setEqMakeOther}
              options={['Modsonic', 'Waygate', 'Kappawave', 'Other']}
            />
          </div>
          <div>
            <label className={labelClass}>Calibration Due</label>
            <input type="date" value={eqCalibrationDue} onChange={e => setEqCalibrationDue(e.target.value)} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Couplant</label>
            <SelectWithOther
              value={eqCouplant} onChange={setEqCouplant}
              otherValue={eqCouplantOther} onOtherChange={setEqCouplantOther}
          ...
          <div>
            ...
            <SelectWithOther
              value={eqCalibBlock} onChange={setEqCalibBlock}
              otherValue={eqCalibBlockOther} onOtherChange={setEqCalibBlockOther}
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-indigo-700 uppercase tracking-wide">Search Unit Details</h2>
          <button type="button" onClick={addSearchUnit} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition-colors">
          ...
          <table className="w-full text-sm border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-600 uppercase">
                <th className="border border-gray-200 px-2 py-2 text-left">Search Unit</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Model</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Angle</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Sr. No.</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Crystal Size</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Wave Mode</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Frequency</th>
              ...
              {searchUnits.map((u, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={u.searchUnit} onChange={e => updateSearchUnit(idx, 'searchUnit', e.target.value)} className={inputClass} placeholder="e.g. Modsonic" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={u.model} onChange={e => updateSearchUnit(idx, 'model', e.target.value)} className={inputClass}>
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={u.angle} onChange={e => updateSearchUnit(idx, 'angle', e.target.value)} className={inputClass}>
                      <option value="">Select...</option>
                      <option>T/R</option>
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={u.srNo} onChange={e => updateSearchUnit(idx, 'srNo', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1 min-w-[140px]">
                    <SelectWithOther
                      value={u.crystalSize} onChange={v => updateSearchUnit(idx, 'crystalSize', v)}
                      otherValue={u.crystalSizeOther} onOtherChange={v => updateSearchUnit(idx, 'crystalSizeOther', v)}
                      options={['Ø5mm', 'Ø10mm', '8x9 mm', '20x22 mm', 'Other']}
                    />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={u.waveMode} onChange={e => updateSearchUnit(idx, 'waveMode', e.target.value)} className={inputClass}>
                      <option value="">Select...</option>
                  ...
                  <td className="border border-gray-200 px-1 py-1 min-w-[130px]">
                    <SelectWithOther
                      value={u.frequency} onChange={v => updateSearchUnit(idx, 'frequency', v)}
                      otherValue={u.frequencyOther} onOtherChange={v => updateSearchUnit(idx, 'frequencyOther', v)}
                      options={['2 MHz', '4 MHz', '5 MHz', 'Other']}
                    />
                  </td>
                  <td className="border border-gray-200 px-1 py-1 text-center">
                    {searchUnits.length > 1 && (
                      <button type="button" onClick={() => removeSearchUnit(idx)} className="text-red-400 hover:text-red-600">
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-indigo-700 uppercase tracking-wide">Technique Details</h2>
          <button type="button" onClick={addTechnique} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition-colors">
            <Plus className="w-3 h-3" /> Add Row
          </button>
        </div>
          ...
          <table className="w-full text-sm border-collapse min-w-[800px]">
            ...
            <tbody>
              {techniques.map((t, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={t.searchUnit} onChange={e => updateTechnique(idx, 'searchUnit', e.target.value)} className={inputClass} placeholder="e.g. Modsonic" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={t.angle} onChange={e => updateTechnique(idx, 'angle', e.target.value)} className={inputClass}>
                      <option value="">Select...</option>
                      <option>Normal (0°)</option>
                      <option>45°</option>
                      <option>60°</option>
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={t.srNo} onChange={e => updateTechnique(idx, 'srNo', e.target.value)} className={inputClass} />
                  </td>
                  <td className="border border-gray-200 px-1 py-1 min-w-[140px]">
                    <SelectWithOther
                      value={t.crystalSize} onChange={v => updateTechnique(idx, 'crystalSize', v)}
                      otherValue={t.crystalSizeOther} onOtherChange={v => updateTechnique(idx, 'crystalSizeOther', v)}
                      options={['Ø5mm', 'Ø10mm', '8x9 mm', '20x22 mm', 'Other']}
                    />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <select value={t.waveMode} onChange={e => updateTechnique(idx, 'waveMode', e.target.value)} className={inputClass}>
                      <option value="">Select...</option>
                      <option>Longitudinal</option>
                      <option>Shear</option>
                    </select>
                  ...
                  <td className="border border-gray-200 px-1 py-1 min-w-[130px]">
                    <SelectWithOther
                      value={t.frequency} onChange={v => updateTechnique(idx, 'frequency', v)}
                      otherValue={t.frequencyOther} onOtherChange={v => updateTechnique(idx, 'frequencyOther', v)}
                      options={['2 MHz', '4 MHz', '5 MHz', 'Other']}
                    />
                  </td>
                  <td className="border border-gray-200 px-1 py-1 text-center">
                    {techniques.length > 1 && (
                      <button type="button" onClick={() => removeTechnique(idx)} className="text-red-400 hover:text-red-600">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    )}
                  </td>
        ...
        <div className="max-w-sm">
          ...
          <SelectWithOther
            value={utMethod} onChange={setUtMethod}
            otherValue={utMethodOther} onOtherChange={setUtMethodOther}
            options={['Pulse Echo – Contact', 'Through Transmission', 'Immersion', 'Other']}
          />
        </div>
      </div>
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
          ...
          <button type="button" onClick={addObs} className="flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 rounded-lg px-3 py-1.5 hover:bg-indigo-50 transition-colors">
            <Plus className="w-3 h-3" /> Add Row
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-gray-50 text-xs text-gray-600 uppercase">
                ...
                <th className="border border-gray-200 px-2 py-2 text-left">Item Name</th>
                <th className="border border-gray-200 px-2 py-2 text-left">Measured Thickness</th>
            ...
            <tbody>
              {observations.map((row, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.itemName} onChange={e => updateObs(idx, 'itemName', e.target.value)} className={inputClass} placeholder="e.g. Panel" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
                    <input type="text" value={row.measuredThickness} onChange={e => updateObs(idx, 'measuredThickness', e.target.value)} className={inputClass} placeholder="e.g. 12.5" />
                  </td>
                  <td className="border border-gray-200 px-1 py-1">
          ...
          <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <input type="text" value={inspectorQual} onChange={e => setInspectorQual(e.target.value)} className={inputClass} placeholder="e.g. UT NDE Level II" />
              </div>
              <div>
                <label className={labelClass}>Signature</label>
                <input type="text" value={inspectorSig} onChange={e => setInspectorSig(e.target.value)} className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>I.D. No.</label>
                <input type="text" value={inspectorIdNo} onChange={e => setInspectorIdNo(e.target.value)} className={inputClass} placeholder="e.g. 115" />
              </div>
              <div>
                <label className={labelClass}>Date</label>
                <input type="date" value={inspectorDate} onChange={e => setInspectorDate(e.target.value)} className={inputClass} />
              </div>
            </div>
          </div>
          {/* Customer */}
      ...
      <div className="flex items-center justify-end gap-3 pb-8">
        ...
        <button type="button" disabled={saving} onClick={() => handleSubmit('final')}
