...
const RoleBadge: React.FC<{ role: Role }> = ({ role }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${cfg.bg} ${cfg.color}`}
    >
...
const UserModal: React.FC<UserModalProps> = ({
  ...
  const pwdStrength =
    pwdScore <= 1
      ? { label: "Weak", color: "bg-red-500", text: "text-red-600", bars: 1 }
      : pwdScore === 2
        ? {
            label: "Fair",
            color: "bg-orange-400",
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-violet-100/90 text-sm mt-1 max-w-2xl">
                {mode === "create"
                  ? "Fill in details — a welcome email will be sent."
                    ...
                    <div className="mt-2.5 space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex gap-1 flex-1">
                          {[1, 2, 3, 4, 5].map((i) => (
                            <div
                              key={i}
                              className={`h-1.5 flex-1 rounded-full transition-colors ${i <= pwdStrength.bars ? pwdStrength.color : "bg-gray-200"}`}
                            />
                          ))}
                        </div>
                        <span
                          className={`text-xs font-semibold ${pwdStrength.text}`}
                        >
                          {pwdStrength.label}
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                        {(
                          ...
                          >
                            <span
                              className={`w-3.5 h-3.5 rounded-full flex items-center justify-center flex-shrink-0 text-white text-[9px] font-bold ${ok ? "bg-emerald-500" : "bg-gray-300"}`}
                            >
                              {ok ? "✓" : ""}
                            </span>
                            {label}
            ...
            {mode === "edit" && currentUserRole !== "EMPLOYEE" && (
              <div className="md:col-span-2">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <div
                    className={`w-11 h-6 rounded-full transition-colors ${form.isActive ? "bg-emerald-500" : "bg-gray-300"}`}
                  />
...
const DeleteConfirm: React.FC<DeleteConfirmProps> = ({
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200">
      <div
        className={`w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-4 ${type === "hard" ? "bg-red-100" : "bg-amber-100"}`}
      >
        <AlertTriangle
          size={28}
          className={type === "hard" ? "text-red-600" : "text-amber-600"}
        />
      </div>
      <h3 className="text-lg font-bold text-gray-900 text-center">
        {type === "hard" ? "Permanently Delete User" : "Deactivate User"}
      </h3>
      <p className="text-sm text-gray-500 text-center mt-2">
        {type === "hard" ? (
          <>
            This will{" "}
            <strong className="text-red-600">permanently remove</strong>{" "}
            <span className="font-semibold text-gray-700">{user.name}</span> and
            all their data.
          </>
        ) : (
          <>
            This will deactivate{" "}
            <span className="font-semibold text-gray-700">{user.name}</span>'s
            account.
          </>
        )}
      </p>
      <div className="flex gap-3 mt-6">
        ...
        <button
          onClick={onConfirm}
          disabled={loading}
          className={`flex-1 py-2.5 rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2 transition-all disabled:opacity-60 ${type === "hard" ? "bg-red-600 hover:bg-red-700 shadow-lg shadow-red-200" : "bg-amber-500 hover:bg-amber-600 shadow-lg shadow-amber-200"}`}
        >
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl text-sm font-semibold hover:from-violet-700 hover:to-purple-700 transition-all shadow-lg shadow-violet-200"
        >
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center text-white shadow-sm`}
                >
                  <Icon size={18} />
                </div>
                <div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          />
          <input
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 placeholder-gray-400 outline-none focus:bg-white transition-all"
        ...
        <div className="flex gap-2">
          <select
            className="px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 outline-none"
            value={roleFilter}
      ...
      <div className="grid grid-cols-1 gap-4 sm:hidden">
        {loading && users.length === 0
          ? [...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-40 bg-gray-50 rounded-2xl animate-pulse"
              />
            ))
          : paginatedUsers.map((user) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${getGradient(user.name)} flex items-center justify-center text-white font-bold text-lg shadow-sm`}
                    >
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="p-2.5 rounded-xl border border-gray-100 text-gray-400 hover:text-violet-600"
                  >
                    <Edit2 size={18} />
                  </button>
                  <button
                    onClick={() =>
                      user.isActive
                        ? setDeleteTarget({ user, type: "soft" })
                        : handleActivate(user)
                    }
                    className={`p-2.5 rounded-xl border border-gray-100 ${user.isActive ? "text-amber-500" : "text-emerald-500"}`}
                  >
                    {user.isActive ? (
                      <ShieldOff size={18} />
                    ) : (
                  ...
                  {currentUserRole === "SUPER_ADMIN" &&
                    user.role !== "SUPER_ADMIN" && (
                      <button
                        ...
                        className="p-2.5 rounded-xl border border-gray-100 text-red-500"
                      >
                        <Trash2 size={18} />
                      </button>
                    )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/70 text-gray-500">
                <th className="text-left px-4 py-4 font-bold uppercase tracking-wider text-[10px]">
                  User
                </th>
                <th className="text-left px-4 py-4 font-bold uppercase tracking-wider text-[10px]">
                  Contact
                ...
                <th className="text-center px-4 py-4 font-bold uppercase tracking-wider text-[10px]">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                : paginatedUsers.map((user) => (
                    ...
                    >
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-9 h-9 rounded-xl bg-gradient-to-br ${getGradient(user.name)} flex items-center justify-center text-white font-bold text-sm shadow-sm`}
                          >
                            {user.name.charAt(0).toUpperCase()}
                          </div>
                          <div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {user.isActive ? (
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg text-gray-400 hover:text-emerald-500"
                                title="Activate"
                              >
                                <ShieldCheck size={16} />
                              </button>
                            ...
                            {currentUserRole === "SUPER_ADMIN" &&
                              user.role !== "SUPER_ADMIN" && (
                                <button
                                  onClick={() =>
                                    setDeleteTarget({ user, type: "hard" })
                                  }
