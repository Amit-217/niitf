...
const BatchModal: React.FC<ModalProps> = ({
  batch,
  courses,
  onClose,
  onSaved,
  ...
  const [startDate, setStartDate] = useState(
    batch ? batch.startDate.slice(0, 10) : "",
  );
  const [endDate, setEndDate] = useState(
    batch?.endDate ? batch.endDate.slice(0, 10) : "",
  );
  const [status, setStatus] = useState<"Upcoming" | "Running" | "Completed">(
        ...
        <div className="bg-gradient-to-r from-violet-600 to-indigo-700 px-6 py-5 rounded-t-2xl flex items-start justify-between">
          <div>
            ...
            <p className="text-violet-200 text-sm mt-0.5">
              {batch
                ? "Update schedule or status."
                : "Register a new batch for instruction."}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g. NDT Level 2 (2026)"
    ...
    >
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400">{batch.batchId}</p>
            </div>
          </div>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-6 w-32 bg-white border rounded-xl shadow-xl py-1 z-10 text-xs">
                ...
                <button
                  ...
                  className="w-full text-left p-2 hover:bg-red-50 text-red-600 flex items-center gap-2"
                >
        ...
        <div className="flex gap-2 mb-4">
          <span className="bg-violet-50 text-violet-700 text-[10px] font-bold p-1 rounded border border-violet-100 truncate flex-1 uppercase text-center">
            {batch.courseId?.courseName}
          </span>
          <span className="bg-blue-50 text-blue-700 text-[10px] font-bold p-1 rounded border border-blue-100 flex items-center gap-1">
            <Users size={10} /> {batch.studentCount}
          </span>
        </div>
...
export const BatchesPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "All" | "Upcoming" | "Running" | "Completed"
  >("All");
  ...
  const handleToggleActive = async (b: Batch) => {
    try {
      await api.put(`/batches/${b.batchId}`, { isActive: !b.isActive });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <h1 className="text-2xl font-bold flex items-center gap-2 text-gray-900">
          <Layers className="text-emerald-500" /> Batches
        </h1>
        <div className="flex gap-2">
          <button
            onClick={fetchData}
            className="p-2.5 border rounded-xl text-gray-500 hover:text-violet-600 hover:border-violet-300 hover:bg-violet-50 transition-all"
          >
            <RefreshCw size={16} />
          </button>
          <button
        ...
        <div className="flex gap-1.5 bg-gray-100 p-1 rounded-xl overflow-x-auto">
          {(["All", "Upcoming", "Running", "Completed"] as const).map((t) => (
            <button
              ...
              className={`px-4 py-1.5 text-xs font-bold rounded-lg ${statusFilter === t ? "bg-white shadow-sm" : "text-gray-500"}`}
            >
              {t}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="py-20 text-center text-gray-500">No batches found</div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm text-center">
            ...
            <p className="text-sm text-gray-500 mb-6">
              "{deleteTarget.batchName}" will be removed.
            </p>
